...
namespace ADVT_ERP
{
    ...
    public partial class ReceiptReport : UserControl
    {
        ...
        public ReceiptReport(string usertype)
        {
            firstrun = true;
            InitializeComponent();
            firstrun = false;
        ...
        private void btnApplyFilter_Click(object sender, RoutedEventArgs e)
        {
            ...
            refreshGrid(fromdate, todate, district, location, agency);
        }

        public void refreshGrid(string fromdate = "", string todate = "",  string district = "", string location = "", string agency = "")
        {
            string qry = @"SELECT top 2000 pr.ReceiptNo, FORMAT(pr.ReceiptDate, 'dd/MM/yyyy') as ReceiptDate, lc.LocationName, ag.AgencyName, pr.ClientName, pr.Amount, pr.PymentMode, pr.PymentStatus, pr.ChequeNoTxnId, FORMAT(pr.ChequeTxnDate, 'dd/MM/yyyy') as TxnDate from PaymentReceipt pr left join Master_Agency ag on pr.AgencyId = ag.Id left join Master_Location lc on pr.LocationId = lc.Id where pr.IsCanceled = 'N' ";

            if(fromdate.Length > 2 && todate.Length > 2)
            {
                qry += " and pr.ReceiptDate between '" + fromdate + "' and '"+ todate +"' ";
            }

            if(district != "" && district != "0")
            {
                qry += " and pr.DistrictId = '" + district + "'";
            }

            if (location != "" && location != "0")
            {
                qry += " and pr.LocationId = '" + location + "'";
            }

            if (agency != "" && agency != "0")
            {
                qry += " and pr.AgencyId = '" + agency + "'";
            }
        ...
        private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                foreach (DataRowView row in grdInvoiceList.SelectedItems)
                {
                    ...
                    ReceiptNo = MyRow["ReceiptNo"].ToString();

                }

                var print = new ViewReceipt(ReceiptNo);
